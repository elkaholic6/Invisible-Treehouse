...
import { playPause, setActiveSong, setNewIndex } from '../redux/features/playerSlice';

const OwnedNFTsCards = ({ image, minterContract, quantityOwned, quantityListed, tokenId, listingCreator, listingId, currentlyListed, pricePerToken, isPlaying, activeSong, songData, animation_url, songList, name, artist, i, id, songArr }) => {
        ...
        <div className='flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer'>
            ...
            <div className='mt-4 flex flex-col'>
                ...
                <p className='text-sm truncate flex flex-row justify-between'>
                    <span className='text-blue-300 mr-2 mt-4'>Your Ownings:</span>
                    <span className='font-bold text-white mt-4 text-right'>{quantityOwned}</span>
                </p>
                {currentlyListed ? (
                <p className='text-sm truncate flex flex-row justify-between'>
                    <span className='text-blue-300 mr-2 mt-1'>Your listings:</span>
                    <span className='font-bold text-white mt-1 text-right'>{quantityListed}</span>
                </p>
                ) : (
                <p className='text-sm truncate flex flex-row justify-between'>
                    <span className='text-red-300 mt-1 text-right'>Item Not Currently Listed</span>
                </p>
                )}
            </div>
        </div>
