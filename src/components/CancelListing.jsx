...
function CancelListing({ minterContractFromNFTDetails, imageFromNFTDetails, listingIdFromNFTDetails }) {
    ...
    async function cancelTheListing() {
        try {
            setError(null); 
            setIsCancelling(true);
            await marketplaceContract.cancelListing(minterContractFromNFTDetails, listingIdFromNFTDetails, quantity);
            setTimeout(() => {
                setOpen(false);
                resetFields();
            }, 2000); 
...
<div className="flex flex-row justify-start items-center">
    {isWalletConnected ? (
        ...
        type="button"
        className="flex flex-initial w-32 btn bg-[#c33d2e] text-white hover:bg-[#b42f2c] rounded-md font-normal py-1 pl-3.5 mr-3">
        Cancel Listing
        </button>
        ...
        <div className='bg-black bg-opacity-40 absolute inset-0 bottom-28 flex justify-center items-center z-10'>
            <div className='bg-[#f3e8db] rounded-lg w-96 h-[90%] sm:h-fit overflow-y-auto'>
                <div className='flex justify-between items-center'>
                        <h4 className='text-2xl flex justify-center p-2 ml-4 font-medium text-black'>Cancel Listing</h4>
                        <RiCloseLine 
